...
const Products = () => {
  ...
  useEffect(() => {
    dispatch(getData(paramObject));
  }, [page, order, search, searchParams])
    ...
    <div className='flex mt-12'>
      ...
      <div className='w-[80%] ms-auto'>
        <div className='m-8 flex items-center justify-around'>
          <select disabled={searchParams.getAll('category').length > 0} className="select select-info w-full max-w-xs" onChange={(e) => setCategory(e.target.value)}>
        ...
        <div className='flex flex-wrap justify-around' style={{ backgroundColor: theme === 'light' ? 'black' : 'white', color: theme === 'light' ? 'white' : 'black' }}>
